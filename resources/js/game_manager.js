function GameManager({onGameOver, onStartTurn, onLineClear}){
    ...
    gridCanvas.width = 20 * grid.columns;
    gridCanvas.height = 20 * (grid.rows - 2);

    function gameOver(){
        onGameOver?.({resetFunction:reset});
    }
    ...
    function reset() {
        ...
        grid = new Grid(window.ROWS, window.COLS);
    ...
    function startTurn(){
        ...
        onStartTurn?.({stats:getStats(), resetFunction:reset});
    ...
    function endTurn(){
        ...
        onLineClear?.({amount:linesCleared, resetFunction:reset });
